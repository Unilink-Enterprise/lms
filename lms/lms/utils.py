...
def get_paystack_api():
	settings = frappe.get_single("Paystack Integration Settings")
	paystack_key = settings.api_key
	paystack_secret = settings.get_password("api_secret", raise_exception=True)
...
def create_paystack_order(api_secret, amount, currency, callback_url):
	try:
		if not frappe.session.user:
			frappe.throw("Please login before buying")
			return
		...
		res = requests.post("https://api.paystack.co/transaction/initialize", json=data, headers= {"Content-Type": "application/json", "Authorization": f"Bearer {api_secret}"})
		res = res.json()
...
@frappe.whitelist()
def get_paystack_payment_options(doctype, docname, phone, country, callback_url):
	if not frappe.db.exists(doctype, docname):
		frappe.throw(_("Invalid document provided."))
	validate_phone_number(phone, True)
	details = get_details(doctype, docname)
	...
	paystack_api = get_paystack_api()
 
	order = create_paystack_order(paystack_api["paystack_secret"], details.amount, details.currency, callback_url)
 
	options = {
		"key_id": paystack_api["paystack_key"],
		"name": frappe.db.get_single_value("Website Settings", "app_name"),
		"description": _("Payment for {0} course").format(details["title"]),
		"order_id": order["data"]["reference"],
...
@frappe.whitelist()
def save_user_address(address):
    address_doc_name = save_address(address)
...
def record_paystack_payment(address, response, doctype, docname, billing_name, source):
...
@frappe.whitelist()
def verify_paystack_transaction(reference, doctype, docname, billing_name, source):
    ps_settings = frappe.get_single("Paystack Integration Settings")
    api_secret = ps_settings.get_password("api_secret")
    
    res = requests.get(f"https://api.paystack.co/transaction/verify/{reference}", headers= {"Content-Type": "application/json", "Authorization": f"Bearer {api_secret}"})
    res = res.json()
    ...
    if(res["status"]):
        ...
        if exists:
            address_doc = frappe.get_last_doc("Address", filters=filters)
            payment = record_paystack_payment(address_doc, res, doctype, docname, billing_name, source)
            if doctype == "LMS Course":
                return create_membership(docname, payment)
            else:
                return add_student_to_batch(docname, payment)
